/* ============================================================
 * lunar.js —— 公历/农历互转 + 农历中文显示
 *
 * 转换核心改编自 Lunar-Solar-Calendar-Converter
 *   https://github.com/isee15/Lunar-Solar-Calendar-Converter
 *   （MIT License，Copyright isee15），支持 1888 – 2110 年。
 * 上层封装与中文月/日名由本工程补充。
 * ============================================================ */
'use strict';
...
function LunarSolarConverter() {
  ...
  this.SolarFromInt = function (g) {
    ...
    if (ddd < 0) { y--; ddd = g - (365 * y + parseInt(y / 4) - parseInt(y / 100) + parseInt(y / 400)); }
    var mi = parseInt((100 * ddd + 52) / 3060);
    var mm = (mi + 2) % 12 + 1;
    y = y + parseInt((mi + 2) / 12);
    var dd = ddd - parseInt((mi * 306 + 5) / 10) + 1;
    var solar = new Solar();
    solar.solarYear = parseInt(y); solar.solarMonth = parseInt(mm); solar.solarDay = parseInt(dd);
    return solar;
  };
  ...
  this.LunarToSolar = function (lunar) {
    ...
    var loopend = leap;
    if (!lunar.isleap) {
      if (lunar.lunarMonth <= leap || leap === 0) { loopend = lunar.lunarMonth - 1; } else { loopend = lunar.lunarMonth; }
    }
  ...
  this.SolarToLunar = function (solar) {
    ...
    if (this.solar_1_1[index] > data) { index--; }
    ...
    for (var i = 0; i < 13; i++) {
      ...
      if (offset > dm) { lunarM++; offset -= dm; } else { break; }
    }
    ...
    if (leap !== 0 && lunarM > leap) {
      ...
      if (lunarM === leap + 1) { lunar.isleap = true; }
    }
...
(function () {
  'use strict';
  const conv = new LunarSolarConverter();
  const MIN_Y = 1888, MAX_Y = 2110;
  ...
  function inRange(y) { return y >= MIN_Y && y <= MAX_Y; }

  function leapMonthOf(lunarYear) {
    if (!inRange(lunarYear)) return 0;
    const days = conv.lunar_month_days[lunarYear - conv.lunar_month_days[0]];
    return conv.GetBitInt(days, 4, 13);
  }

  function monthDayCount(lunarYear, lunarMonth, isLeap) {
    if (!inRange(lunarYear) || lunarMonth < 1 || lunarMonth > 12) return 0;
    const days = conv.lunar_month_days[lunarYear - conv.lunar_month_days[0]];
    // 位域：高 4 位闰月，低 13 位为正月到腊月（含闰月）每月是否 30 天
    let pos = lunarMonth; // 正常月
    if (isLeap) { if (leapMonthOf(lunarYear) !== lunarMonth) return 0; pos = lunarMonth + 1; }
    else if (leapMonthOf(lunarYear) !== 0 && lunarMonth > leapMonthOf(lunarYear)) { pos = lunarMonth + 1; }
    return conv.GetBitInt(days, 1, 12 - (pos - 1)) === 1 ? 30 : 29;
  }

  function lunarToSolar(lunarYear, lunarMonth, lunarDay, isLeap) {
    if (!inRange(lunarYear) || lunarMonth < 1 || lunarMonth > 12 || lunarDay < 1) return null;
    const days = monthDayCount(lunarYear, lunarMonth, isLeap);
    if (days === 0 || lunarDay > days) return null;
    const l = new Lunar();
    l.lunarYear = lunarYear; l.lunarMonth = lunarMonth; l.lunarDay = lunarDay; l.isleap = !!isLeap;
    const s = conv.LunarToSolar(l);
    return { y: s.solarYear, m: s.solarMonth, d: s.solarDay };
  }

  function solarToLunar(y, m, d) {
    if (!inRange(y)) return null;
    const s = new Solar(); s.solarYear = y; s.solarMonth = m; s.solarDay = d;
    const l = conv.SolarToLunar(s);
    return {
      lYear: l.lunarYear, lMonth: l.lunarMonth, lDay: l.lunarDay, isLeap: l.isleap
    };
  }

  function monthName(month, isLeap) { return (isLeap ? '闰' : '') + (MONTH_NAMES[month - 1] || ''); }

  function dayName(day) {
    if (day < 1 || day > 30) return '';
    const D1 = ['初一', '初二', '初三', '初四', '初五', '初六', '初七', '初八', '初九', '初十'];
    const D2 = ['十一', '十二', '十三', '十四', '十五', '十六', '十七', '十八', '十九', '二十'];
    const D3 = ['廿一', '廿二', '廿三', '廿四', '廿五', '廿六', '廿七', '廿八', '廿九', '三十'];
    return day <= 10 ? D1[day - 1] : day <= 20 ? D2[day - 11] : D3[day - 21];
  }

  function zodiacOf(lunarYear) { return ZODIAC[((lunarYear - 4) % 12 + 12) % 12]; }

  function ganzhiOf(lunarYear) {
  ...
  window.DM = window.DM || {};
  window.DM.lunar = {
    MIN_Y, MAX_Y, MONTH_NAMES,
    inRange, leapMonthOf, monthDayCount,
    lunarToSolar, solarToLunar,
    monthName, dayName, zodiacOf, ganzhiOf
  };
})();
